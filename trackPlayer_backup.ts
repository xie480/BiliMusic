...
/** 切歌互斥锁：保证连续快速切歌时，队列操作的原子性，防止索引错乱 */
const skipMutex = new Mutex();
...
export async function PlaybackService() {
  ...
  // 【终极修复】锁屏/通知栏切歌：放弃相对跳转，采用精准定点跳转，并引入互斥锁防止并发错乱
  TrackPlayer.addEventListener(Event.RemoteNext, async () => {
    await skipMutex.acquire();
    usePlayerStore.getState().setResolving(true);
    ...
    try {
      ...
      if (nextIndex >= queue.length) {
        // 已经是最后一首，停止或循环（根据您的业务逻辑，这里假设停止）
        await TrackPlayer.stop();
        return;
      }

      const nextTrack = queue[nextIndex];
      const urlStr = String(nextTrack.url || '');
      
      // 如果下一首是占位符，我们手动接管解析和跳转流程
      if (urlStr.startsWith('placeholder://') || urlStr.includes('silence_10s.wav#')) {
        LoggerService.info('TrackPlayer', 'RemoteNext', '拦截切歌：手动接管占位符解析与跳转');
        
        // 【核心唤醒逻辑】在发起网络请求前，强制调用 play() 唤醒 Foreground Service
        // 即使当前处于暂停状态，短暂的 play 也能解除 Android 系统的网络休眠限制
        await TrackPlayer.play().catch(() => {});

        // 1. 提取 BVID 和 CID
        let rawId = '';
        if (urlStr.startsWith('placeholder://')) {
          rawId = urlStr.replace('placeholder://', '');
        } else {
          rawId = urlStr.split('#')[1] || '';
        }
        const [bvid, cidStr] = rawId.split('-');
        const cid = cidStr ? parseInt(cidStr, 10) : undefined;
        
        // 2. 获取真实 URL (复用 audioService 逻辑)
        ...
        // 4. 精准替换与跳转 (核心时序控制)
        // 先在 nextIndex + 1 的位置插入真实轨道
        await TrackPlayer.add(realTrack, nextIndex + 1);
        // 直接精准跳转到刚刚插入的真实轨道
        await TrackPlayer.skip(nextIndex + 1);
        // 播放
        await TrackPlayer.play();
        // 最后安全地移除旧的占位符
      ...
      LoggerService.error('TrackPlayer', 'RemoteNext', '切歌失败:', error);
      usePlayerStore.getState().setPlaybackError('切歌失败，请检查网络');
    } finally {
      usePlayerStore.getState().setResolving(false);
      skipMutex.release();
    }
  ...
  TrackPlayer.addEventListener(Event.RemotePrevious, async () => {
    ...
    try {
      ...
      const urlStr = String(prevTrack.url || '');
      
      if (urlStr.startsWith('placeholder://') || urlStr.includes('silence_10s.wav#')) {
        LoggerService.info('TrackPlayer', 'RemotePrevious', '拦截切歌：手动接管占位符解析与跳转');
        ...
        // 在 prevIndex + 1 的位置插入真实轨道 (即当前轨道之前)
        await TrackPlayer.add(realTrack, prevIndex + 1);
        // 精准跳转
        await TrackPlayer.skip(prevIndex + 1);
        await TrackPlayer.play();
        // 移除旧的占位符
      ...
      LoggerService.error('TrackPlayer', 'RemotePrevious', '切歌失败:', error);
      usePlayerStore.getState().setPlaybackError('切歌失败，请检查网络');
    } finally {
      usePlayerStore.getState().setResolving(false);
      skipMutex.release();
  ...
  TrackPlayer.addEventListener(Event.PlaybackActiveTrackChanged, async (e) => {
    ...
    // 【新增】相邻轨道预水合 (Hydration)：直接替换原生队列中的下一首
    const queue = await TrackPlayer.getQueue();
    const nextIndex = actualIndex + 1;
    if (nextIndex < queue.length) {
      const nextTrack = queue[nextIndex];
      if (String(nextTrack.url).startsWith('placeholder://')) {
        LoggerService.info('TrackPlayer', 'Hydration', `静默水合下一首轨道 (index: ${nextIndex})`);
        // 后台静默解析并替换，不影响当前播放，确保自然播完时无缝衔接
        lazyResolve(nextIndex, { version: capturedVersion, autoPlay: false }).catch(() => {});
      }
    }
  });
  ...
  TrackPlayer.addEventListener(Event.PlaybackError, async (error) => {
    ...
    try {
      ...
      if (
        ...
        (activeTrack.url.startsWith('placeholder://') || activeTrack.url.includes('silence_10s.wav#'))
      ) {
        LoggerService.info(
          'TrackPlayer',
          'PlaybackError',
    ...
    // 剥离了常规业务逻辑后，这里只处理真正的网络异常或 URL 失效
    // 不再自动 skipToNext，避免死循环
...
export async function playSpecificPart(bvid: string, cid: number, partTitle: string) {
  ...
  const silenceUrl = `file://${RNFS.MainBundlePath}/silence_10s.wav`;
  const placeholderUrl = `${silenceUrl}#${bvid}-${cid}`;
  const quality = useSettingsStore.getState().quality;
  ...
  if (expandMultiPart) {
    ...
    const existingIndex = currentQueue.findIndex(
      t => {
        ...
        return t.id === bvid && ((t as any).cid === cid || u === placeholderUrl || u === `placeholder://${bvid}-${cid}`);
      }
    );
